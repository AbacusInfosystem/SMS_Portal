...
{
   public class ThirdPartyVendorRepo
    {
        SQLHelper _sqlHelper;

        public ThirdPartyVendorRepo()
        {
        ...
        {
            _sqlHelper.ExecuteNonQuery(Set_Values_In_Vendor(vendor, user_Id, entity_Id), StoreProcedures.Insert_Vendor_Sp.ToString(), CommandType.StoredProcedure);
        }

        public void Update_Vendor(ThirdPartyVendorInfo vendor , int user_Id)
        {
            _sqlHelper.ExecuteNonQuery(Set_Values_In_Vendor(vendor, user_Id,0), StoreProcedures.Update_Vendor_Sp.ToString(), CommandType.StoredProcedure);
        }

        private List<SqlParameter> Set_Values_In_Vendor(ThirdPartyVendorInfo vendor, int user_Id, int entity_Id)
        {
            List<SqlParameter> sqlParams = new List<SqlParameter>();
            if (vendor.Vendor_Id != 0)
            {
                sqlParams.Add(new SqlParameter("@Third_Party_Vendor_Id", vendor.Vendor_Id));
            }
            if (vendor.Vendor_Id == 0)
            {
                sqlParams.Add(new SqlParameter("@Vendor_Id", entity_Id));
            }
            sqlParams.Add(new SqlParameter("@Third_Party_Vendor_Name", vendor.Vendor_Name));
        ...
        {
            List<ThirdPartyVendorInfo> Vendors = new List<ThirdPartyVendorInfo>();
            List<SqlParameter> sqlParamList = new List<SqlParameter>();
            sqlParamList.Add(new SqlParameter("@Entity_Id", entity_Id));
            DataTable dt = _sqlHelper.ExecuteDataTable(sqlParamList, StoreProcedures.Get_Vendor_Sp.ToString(), CommandType.StoredProcedure);
        ...
        public List<ThirdPartyVendorInfo> Get_Vendor_By_Id_List(int vendor_Id, ref PaginationInfo pager)
        {
            ...
            List<ThirdPartyVendorInfo> Vendor = new List<ThirdPartyVendorInfo>();
            DataTable dt = _sqlHelper.ExecuteDataTable(sqlParamList, StoreProcedures.Get_Vendor_By_Id_Sp.ToString(), CommandType.StoredProcedure);
            foreach (DataRow dr in CommonMethods.GetRows(dt, ref pager))
            {
        ...
        public bool Check_Existing_Vendor(string vendor_Name)
        {
            ...
            sqlParam.Add(new SqlParameter("@Third_Party_Vendor_Name", vendor_Name));
        ...
        public void Insert_Vendor_Bank_Details(ThirdPartyVendorInfo vendor,int user_Id)
        {
            foreach (var item in vendor.BankDetailsList)
            {
                ...
                sqlparam.Add(new SqlParameter("@Third_Party_Vendor_Id", vendor.Vendor_Id));
        ...
        public List<Bank_Details> Get_Vendor_Bank_Details(int vendor_Id)
        {
            ...
            sqlparam.Add(new SqlParameter("@Third_Party_Vendor_Id", vendor_Id));
        ...
        public List<ProductInfo> Get_Productmapping(int vendor_Id,int entityId)
        {
            //Pager.PageSize = 20;
            List<ProductInfo> products = new List<ProductInfo>();
            List<SqlParameter> sqlParams = new List<SqlParameter>();
            sqlParams.Add(new SqlParameter("@Entity_Id", entityId));
            sqlParams.Add(new SqlParameter("@Third_Party_Vendor_Id", vendor_Id));
        ...
        private List<SqlParameter> Set_Values_In_Vendor_Profile(ThirdPartyVendorInfo vendor, int user_Id)
        {
            List<SqlParameter> sqlParams = new List<SqlParameter>();
