    ...
    var invoiceViewModel =
        {
            ...
            Cookies:
                {
                    Entity_Id: $('#hdnEntity_Id').val()
                }             
        }

    $('#divSearchGridOverlay').show();

    CallAjax("/invoice/get-brand-invoice/", "json", JSON.stringify(invoiceViewModel), "POST", "application/json", false, Bind_Invoices_Grid, "", null);
}
...
function Bind_Invoices_Grid(data)
{
    ...
    if (data.Invoices.length > 0)
    {
        for (i = 0; i < data.Invoices.length; i++)
        {
            ...
            htmlText += data.Invoices[i].Amount == null ? "" : data.Invoices[i].Amount;
